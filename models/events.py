...
class Event(Base):
    ...
    entities = Column(JSON, comment="实体信息JSON")
    regions = Column(JSON, comment="地域信息JSON")
    keywords = Column(JSON, comment="关键词JSON数组")
...
class NewsEventRelation(Base):
    ...
    __tablename__ = "news_event_relations"
    ...
    event_id = Column(Integer, ForeignKey("events.id", ondelete="CASCADE"), nullable=False, comment="事件ID")
...
class EventHistoryRelation(Base):
    ...
    __tablename__ = "event_history_relations"
    
    id = Column(Integer, primary_key=True, autoincrement=True, comment="关联主键")
    parent_event_id = Column(Integer, ForeignKey("events.id", ondelete="CASCADE"), nullable=False, comment="父事件ID")
    child_event_id = Column(Integer, ForeignKey("events.id", ondelete="CASCADE"), nullable=False, comment="子事件ID")
