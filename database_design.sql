...
CREATE TABLE `events` (
  ...
  `entities` json DEFAULT NULL COMMENT '实体信息JSON：人物、组织、地点等',
  `regions` json DEFAULT NULL COMMENT '地域信息JSON：国家、省份、城市等',
...
CREATE TABLE `news_event_relations` (
  ...
  CONSTRAINT `fk_news_event_news` FOREIGN KEY (`news_id`) REFERENCES `hot_news_base` (`id`) ON DELETE CASCADE,
  CONSTRAINT `fk_news_event_event` FOREIGN KEY (`event_id`) REFERENCES `events` (`id`) ON DELETE CASCADE
...
CREATE TABLE `event_labels` (
  ...
  CONSTRAINT `fk_event_labels_event` FOREIGN KEY (`event_id`) REFERENCES `events` (`id`) ON DELETE CASCADE
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COMMENT='事件标签表';

-- 事件历史关联表
CREATE TABLE `event_history_relations` (
  `id` int(11) unsigned NOT NULL AUTO_INCREMENT COMMENT '关联主键',
  ...
  CONSTRAINT `fk_event_history_parent` FOREIGN KEY (`parent_event_id`) REFERENCES `events` (`id`) ON DELETE CASCADE,
  CONSTRAINT `fk_event_history_child` FOREIGN KEY (`child_event_id`) REFERENCES `events` (`id`) ON DELETE CASCADE
...
CREATE TABLE `news_processing_status` (
  ...
  `news_id` int(11) unsigned NOT NULL COMMENT '新闻ID，关联hot_news_base.id',
  `processing_stage` varchar(50) NOT NULL COMMENT '处理阶段：pending、aggregating、labeling、completed、failed',
  ...
  CONSTRAINT `fk_news_processing_news` FOREIGN KEY (`news_id`) REFERENCES `hot_news_base` (`id`) ON DELETE CASCADE
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COMMENT='新闻处理状态表';

-- 创建索引优化查询性能
-- 复合索引：按时间范围查询未处理新闻
ALTER TABLE `hot_news_base` ADD INDEX `idx_time_range` (`first_add_time`, `last_update_time`);

-- 复合索引：按事件类型和时间查询
ALTER TABLE `events` ADD INDEX `idx_type_time` (`event_type`, `created_at`);

-- 复合索引：按置信度和状态查询
ALTER TABLE `news_event_relations` ADD INDEX `idx_confidence_status` (`confidence_score`, `relation_type`);