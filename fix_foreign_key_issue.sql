...
-- 1. 删除所有带前缀的表（按依赖关系顺序）
DROP TABLE IF EXISTS `hot_aggr_event_history_relations`;
DROP TABLE IF EXISTS `hot_aggr_event_labels`;
DROP TABLE IF EXISTS `hot_aggr_news_event_relations`;
DROP TABLE IF EXISTS `hot_aggr_events`;
DROP TABLE IF EXISTS `hot_aggr_news_processing_status`;
DROP TABLE IF EXISTS `hot_aggr_processing_logs`;
...
-- 新闻处理状态表（正确引用 hot_news_base）
CREATE TABLE `hot_aggr_news_processing_status` (
  ...
  CONSTRAINT `fk_hot_aggr_news_processing_news` FOREIGN KEY (`news_id`) REFERENCES `hot_news_base` (`id`) ON DELETE CASCADE
...
CREATE TABLE `hot_aggr_news_event_relations` (
  ...
  CONSTRAINT `fk_hot_aggr_news_event_news` FOREIGN KEY (`news_id`) REFERENCES `hot_news_base` (`id`) ON DELETE CASCADE,
  CONSTRAINT `fk_hot_aggr_news_event_event` FOREIGN KEY (`event_id`) REFERENCES `hot_aggr_events` (`id`) ON DELETE CASCADE
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COMMENT='新闻事件关联表';

-- 事件标签表
CREATE TABLE `hot_aggr_event_labels` (
  ...
  CONSTRAINT `fk_hot_aggr_event_labels_event` FOREIGN KEY (`event_id`) REFERENCES `hot_aggr_events` (`id`) ON DELETE CASCADE
...
CREATE TABLE `hot_aggr_event_history_relations` (
  ...
  CONSTRAINT `fk_hot_aggr_event_history_parent` FOREIGN KEY (`parent_event_id`) REFERENCES `hot_aggr_events` (`id`) ON DELETE CASCADE,
  CONSTRAINT `fk_hot_aggr_event_history_child` FOREIGN KEY (`child_event_id`) REFERENCES `hot_aggr_events` (`id`) ON DELETE CASCADE
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COMMENT='事件历史关联表';

-- 处理日志表
CREATE TABLE `hot_aggr_processing_logs` (
...
-- 完成提示
SELECT 'Foreign key issue fixed! All tables recreated with correct references.' as message;